...
def info_init_cve(cve_case,binary,target_type="funcall4"):
    global gadgets_info
    gadgets_file = f"../dataset/cve/{cve_case}/gadget.txt"
    gadgets_data = tools.load_gadget(gadgets_file)
    gadgets_dict = tools.create_gadget_dict(gadgets_data)
    controllable_mem_file_dir = f'../dataset/cve/{cve_case}/cyclic_matches.json'
    ...
    (permission_table, regs_init) = tools.get_map_regs(f"../dataset/cve/{cve_case}")

    controllable_regs = eval(open(f"../dataset/cve/{cve_case}/controble_regs.txt",'r').read())
    if "stack_pivot_prep" in controllable_regs:
        controllable_regs.remove("stack_pivot_prep")
    binary_file = f'../dataset/cve/{cve_case}/{binary}'
    try:
