...
def benchmark_test(fold_info):
    ...
    for b_i in range(len(binary_file_list[start:])):
        ...
        if b.endswith(".bin"):
            ...
            # try:
            if find_reg_gadget_chain.main(b[:-4], file_fold, mem_search_depth=mem_search_depth, reg_search_depth=reg_search_depth, res_json=res_json,valid_mem_reg_num=valid_mem_reg_num,mem_reg_timeout=mem_reg_timeout, reg_timout=reg_timeout):
                success += 1
                end_time = time.time()
...
if __name__ == "__main__":
    ...
    for fold_info in benchmark_fold:
        ...
        sink=f"benchmark_res_mmap/{fold_info['system']}.log",  # 日志文件路径
        level="INFO",    # 关闭 DEBUG
        encoding="utf-8"
        )
        benchmark_test(fold_info)
