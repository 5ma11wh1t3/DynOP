...
def read_write_check(instruction, permission_table):
    ...
    if instruction.isMemoryWrite():
        ...
        if not table_check(writeMemory, permission_table, 'w'):
            return False
    return True
def symbol_solve_controble_reg_mem(regs, inst_idx, insts2, controble_addr_base, offset_list, permission_table,address_map,stack_mem={},write_map={}):
    ...
    for addr, values in write_map.items():
            ctx.setConcreteMemoryAreaValue(addr,p64(values['value']))
...
def taint_check(inst_idx, insts2, controble_addr_base, offset_list, regs, address_map, stack_mem={}, write_map={}):
    try:
        output_reg = extract_reg(insts2[inst_idx])[0]
...
def update_regs(inst, address_map, regs, permission_table, stack_mem={}, write_map={}):
    ctx = TritonContext()
    ctx.setArchitecture(ARCH.X86_64)
    ctx = init_reg(ctx, regs)
        ...
        for i in range(len(inst)):
            ...
            for j in range(len(inst_list)):
                ...
                if not read_write_check(instruction, permission_table):
                    return None
                if instruction.isMemoryWrite():
                    write_address = instruction.getStoreAccess()[0][0].getAddress()
                    write_value = ctx.getConcreteMemoryValue(instruction.getStoreAccess()[0][0])
                    write_map[write_address] = {'value':write_value,'length':instruction.getStoreAccess()[0][0].getSize()}
    except:
        pass
    for reg,value in regs.items():
        regs[reg] = ctx.getConcreteRegisterValue(get_reg(ctx, reg))
    return (regs, write_map)
...
def solve_mem_gadget(regs_init,classify_res,controble_addr_base,offset_list_init, permission_table):
            ...
            for j in range(len(a)):
                ...
                for inst_idx in range(len(insts2)):
                    if taint_check(inst_idx, insts2, controble_addr_base, offset_list, regs,address_map):
                        ...
                        continue
                regs_new2 = update_regs(insts2, address_map, regs, permission_table, write_map)
                if regs_new2 == None:
                    break
                (regs_new,write_map) = regs_new2
                regs = regs_new
            if count_i != len(a)-1:
                continue
    ...
    return res_f
def solve_mem_gadget_process(classify_res,regs_init,controble_addr_base,offset_list_init, permission_table, stack_mem={},mode=0):
    res_f = {}
    for i in range(len(classify_res)):
        for a in product(*classify_res[i]):
            # if a ==  ('0x0000000000081033 -> mov rsi, qword ptr [rax + 0x10] ; mov rdi, qword ptr [rax + 0x28] ; call qword ptr [rsi + 0x10]', '0x0000000000084604 -> push rsi ; sti ; jmp qword ptr [rsi + 0xf]', '0x000000000003be42 -> pop rsp ; ret'):
            #     pass
            # else:
            #     continue
            # print(a)
            if len(res_f) > 0 and mode!=0:
                return res_f
            last_address = 0
            offset_list = offset_list_init.copy()
            regs = regs_init.copy()
            ...
            for j in range(len(a)):
                ...
                if last_address != 0:
                    ...
                    count_i += 1
                for inst_idx in range(len(insts2)):
                    if taint_check(inst_idx, insts2, controble_addr_base, offset_list, regs,address_map, stack_mem, write_map):
                        res = symbol_solve_controble_reg_mem(regs, inst_idx, insts2, controble_addr_base, offset_list, permission_table,address_map, stack_mem, write_map)
                        if len(res) == 0:
                            continue
                        address = res[0]
                        offset_list.pop(offset_list.index((address-controble_addr_base)//8))
                ...
                regs_new2 = update_regs(insts2, address_map, regs, permission_table, stack_mem, write_map)
                if regs_new2 == None:
                    break
                (regs_new,write_map) = regs_new2
                regs = regs_new
            ...
            if regs_new != None and len(address_map) != 0:
                ...
                for addr, value in address_map.items():
                    r[(addr-controble_addr_base)//8] = f"{hex(value['value'])}"
                res_f[a]=[r,offset_list,time.time()]
    return res_f

from loguru import logger

def solve_mem_gadget_process_func(regs_init,classify_res,controble_addr_base,offset_list_init, permission_table, num_threads, stack_mem, mode=0):
    ...
    if mode == 0:
        solve_func = partial(solve_mem_gadget_process, regs_init=regs_init,controble_addr_base=controble_addr_base,offset_list_init=offset_list_init, permission_table=permission_table, stack_mem=stack_mem, mode=mode)
        ...
        solve_func = partial(solve_mem_gadget_process, regs_init=regs_init, controble_addr_base=controble_addr_base, offset_list_init=offset_list_init, permission_table=permission_table, stack_mem=stack_mem, mode=mode)
        res = []
        # iterate over chunk results as they complete; stop at the first non-empty result
        with multiprocessing.Pool(processes=num_threads) as pool:
            for chunk_res in pool.imap_unordered(solve_func, chunks):
                if chunk_res:
                    for g, v in chunk_res.items():
                        tmp = {}
                        tmp['gadget_chain'] = g
                        tmp['address_map'] = v[0]
                        tmp['offset_list'] = v[1]
...
if __name__ == '__main__':
    ...
    classify_res = [[['0x0000000000050010 -> mov rsi, qword ptr [rdi + 8] ; mov rdx, rdi ; jmp qword ptr [rsi]'],['0x0000000000050020 -> mov rcx, rdx ; call qword ptr [rcx + 0x30]'], ["0x0000000000050030 -> add rdi, rsi ; jmp qword ptr [rsi + 0x10]"]]]
