...
def find_thread(gadgets_dict,lians,controllable_reg,n,depth,lians_part,first_flag):
    # Removed global all_res usage in favor of returning list of chains
    if n > depth:
        return []
    ...
    for addr, lian in lians_use.items():
        if len(lian) > 0:
            ...
            gadget_str = f"{addr} -> {gadgets_dict[addr]}"
            ...
            if is_sink:
                sinks.append(gadget_str)
            else:
                for reg in regs:
                    # Alternative sink check per register
                    if (((v2v(lian, controllable_reg, reg) or (r2v(lian, controllable_reg, reg))) and v2r(lian, reg, 'rsp')) or ((v2r(lian, controllable_reg, reg) or (r2r(lian, controllable_reg, reg))) and r2r(lian, reg, 'rsp'))) and (v2r(lian,controllable_reg,'rip') or r2r(lian,reg,'rip') or v2r(lian,reg,'rip')):
                        ...
                        if (r2r(lian,controllable_reg,reg) or v2r(lian,controllable_reg,reg)) and (v2r(lian,controllable_reg,'rip') or r2r(lian,controllable_reg,'rip') or r2r(lian,reg,'rip') or v2r(lian,reg,'rip')):
                            transitions[reg].append(gadget_str)
                        elif (reg == 'rsp' and (r2v(lian,controllable_reg,reg) and (v2r(lian,controllable_reg,'rip') or r2r(lian,controllable_reg,'rip') or r2r(lian,reg,'rip')))):
                            transitions[reg].append(gadget_str)
    ...
    for reg, gadgets in transitions.items():
        if not gadgets:
            continue
        sub_chains = find_thread(gadgets_dict, lians, reg, n+1, depth, lians_part, False)
        for sub in sub_chains:
            results.append([gadgets] + sub)
...
def find_thread_func(lians_part, gadgets_dict,lians,controllable_reg,n,depth,):
    # Wrapper to match original signature and return format
    all_chains = find_thread(gadgets_dict,lians,controllable_reg,n,depth,lians_part,True)
    return all_chains
...
def stack_pivot_gadget_chain(gadgets_file,n = 0,depth = 1,controllable_reg = 'rdi', num_threads = 24):
    ...
    res = []
    # Flatten the results
    for process_results in all_res_process:
        if process_results:
            res.extend(process_results)
    return res
...
if __name__ == "__main__":
    ...
    gadgets_file = '/home/rop/my-rop/src_v2/stack_pivot_vs_arc/gadgets/nginx.txt'
    n = 0
    depth = 2
    controllable_reg = 'rdi'
